...
<?php
    ...
    {
        $id = $_GET['edit_id'];
        $stmt_edit = $DB_con->prepare('SELECT title,link,img,size FROM offers WHERE id =:id');
...
$size = $_POST['size'];
        ...
        if(!isset($errMSG))
        {
$stmt = $DB_con->prepare('UPDATE offers SET title=:title,link=:link,  img=:img,size=:size
    WHERE id=:id');
